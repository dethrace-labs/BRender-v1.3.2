...
#define MODU_FACE_COPY_FLAGS \
    (BR_MODU_FACE_COLOURS | BR_MODU_FACE_EQUATIONS)
...
static void SmoothingCreased(br_model* model, br_scalar crease_limit,
    ...
    for (outer = start; outer < end; outer++) {
        ...
        for (inner = start; inner < end; inner++) {
            if ((inner == outer) || ((model->faces[(*outer)->f].smoothing & model->faces[(*inner)->f].smoothing) && BrFVector3Dot(&model->faces[(*inner)->f].n, &o_n) > crease_limit)) {
                BrVector3Accumulate(&(*outer)->n, &model->faces[(*inner)->f].n);
            }
        }
    }
...
static void PrepareGroups(br_model* model) {
    ...
    block_size = PREP_ALIGN(sizeof(struct v11model)) + PREP_ALIGN(ng * sizeof(struct v11group)) +
        ...
        PREP_ALIGN(nf * sizeof(br_colour)) + PREP_ALIGN(nv * sizeof(br_colour)) + PREP_ALIGN(nf * sizeof(br_uint_8)) + nv * sizeof(br_int_16) + nf * sizeof(br_int_16);
...
static void PrepareBoundingRadius(br_model* model) {
    ...
    for (v = 0, vp = model->vertices; v < model->nvertices; v++, vp++) {
        d = BrScalarToFloat(vp->p.v[0]) * BrScalarToFloat(vp->p.v[0]) + BrScalarToFloat(vp->p.v[1]) * BrScalarToFloat(vp->p.v[1]) + BrScalarToFloat(vp->p.v[2]) * BrScalarToFloat(vp->p.v[2]);
...
void BR_PUBLIC_ENTRY BrModelUpdate(br_model* model, br_uint_16 flags) {
     ...
     */
    // if (v1db.renderer && v1db.format_model && !(model->flags & BR_MODF_UPDATEABLE) && (model->flags & BR_MODF_FACES_ONLY)) {
