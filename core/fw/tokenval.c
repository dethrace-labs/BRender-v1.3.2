...
static void templateMakeMap(br_tv_template* template) {
    ...
    map = BrResAllocate(template->res,
        n * (sizeof(*template->map_query_entry) + sizeof(*template->map_set_entry)),
        BR_MEMORY_TOKEN_MAP);
    ...
    br_tv_template_entry* tep) /* Template entry to use for conversion		*/
{
    ...
    convert_to_float:
        for (i = 0; i < t; i++)
            MEM_P(br_float)
            [i] = (br_float)BrFixedToFloat(((br_fixed_ls*)(tv->v.p))[i]);
        break;
    ...
    convert_to_fixed:
        for (i = 0; i < t; i++)
            MEM_P(br_fixed_ls)
            [i] = BrFloatToFixed(((br_float*)(tv->v.p))[i]);
        break;
