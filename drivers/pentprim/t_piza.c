...
static inline void scan_inc(const bool carry, const bool light, const short sbpp) {
    // -- Update current  -- //
    // TODO: This seems incredibly bug prone and possibly wrong???
    ...
    work.pz.current += carry ? work.pz.d_carry : work.pz.d_nocarry;
...
static inline char* scan_forward(const bool light, const bool bump, const uint8_t sbpp, const uint8_t dbpp) {
    ...
    while (start < end) {
        ...
        // Z values are big endian for some reason
        br_uint_8 z_low = ((br_uint_8*)&work.pz.currentpix)[2];
        br_uint_8 z_high = ((br_uint_8*)&work.pz.currentpix)[3];

        br_uint_16 z_currentpix = (z_high << 8) | z_low;
        br_uint_16 z_test_start = (zstart[1] << 8) | zstart[0];

        if (z_test_start < z_currentpix) {
            // z test failed
            // Set transparent to skip
            transparent = true;
        } else {
            // Store z as big endian
            zstart[0] = z_low;  // Store low byte
            zstart[1] = z_high; // Store high byte
        }
        ...
        start += dbpp;
        zstart += 2;
        work.pz.currentpix += work.pz.grad_x;
...
static inline void triangle_render_z2(const bool light, const bool bump, const uint8_t sbpp, const uint8_t dbpp) {
...
#if PARTS & PART_BUMP
void BR_ASM_CALL TrapezoidRenderPIZ2TAN() { triangle_render_z2(false, true, 1, 1); } // Normal map
#endif

#if PARTS & PART_8Z
void BR_ASM_CALL TrapezoidRenderPIZ2TA() { triangle_render_z2(false, false, 1, 1); }
void BR_ASM_CALL TrapezoidRenderPIZ2TIA() { triangle_render_z2(true, false, 1, 1); }
#endif

#if PARTS & PART_15Z
void BR_ASM_CALL TrapezoidRenderPIZ2TA15() { triangle_render_z2(false, false, 2, 2); }
void BR_ASM_CALL TrapezoidRenderPIZ2TIA_RGB_555() { triangle_render_z2(true, false, 1, 2); }
#endif

#if PARTS & PART_24Z
void BR_ASM_CALL TrapezoidRenderPIZ2TA24() { triangle_render_z2(false, false, 3, 3); }
void BR_ASM_CALL TrapezoidRenderPIZ2TIA_RGB_888() { triangle_render_z2(true, false, 1, 3); }
#endif
