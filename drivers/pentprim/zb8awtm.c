...
static inline void PER_SCAN_ZTI(int32_t *halfCount, char wrap_flag, uint32_t *minorX, uint32_t *d_minorX)
{
    ...
    ecx.uint_val = ((uint32_t *)&workspace.d_i_y_0)[2 * edi.int_val];
	// mov edx,workspaceA.su
    edx.uint_val = workspaceA.su;
	// mov workspace.s_i,ecx
    workspace.s_i = ecx.uint_val;
	// mov ebp,[workspace.d_z_y_0+8*edi]
    ebp.uint_val = ((uint32_t *)&workspace.d_z_y_0)[2 * edi.int_val];
	// mov workspace.c_i,ecx
    workspace.c_i = ecx.uint_val;
	// mov ecx,workspace.s_z
    ecx.uint_val = workspace.s_z;
...
static inline void DRAW_ZTI_I8(uint32_t *minorX, uint32_t *d_minorX, char direction, int32_t *halfCount, char wrap_flag, tFog_enabled fogging, tBlend_enabled blend)
{
    ...
    ebx.uint_val = workspace.s_i;
    // 	mov workspace.c_z,ecx
    workspace.c_z = ecx.uint_val;
    // mov workspace.c_i,ebx
    workspace.c_i = ebx.uint_val;
...
drawPixel:
    ...
    // else
        ...
        if (blend == eBlend_yes) {
            ...
            ebx.bytes[0] = ((uint8_t *)work.shade_table)[ebx.uint_val];
            //     mov eax,work.blend_table
            //     mov bh,[edi+ecx]
            ebx.bytes[1] = ((uint8_t *)work.colour.base)[edi.uint_val + ecx.uint_val];
            //     mov bl,[eax+ebx]
...
noPlot:
    ...
    ADD_D(eax.uint_val, workspace.d_i_x, direction);
    // 	add_d edx,workspaceA.dvxf,direction
    if(direction == DRAW_LR) {
        ADD_AND_SET_CF(edx.uint_val, workspaceA.dvxf);
    } else {
