 ...
 */
br_device *DeviceSDL2Allocate(char *identifier)
{
    br_device *self;

    if(SDL_InitSubSystem(SDL_INIT_VIDEO) < 0) {
        BrFailure("Initialisation failure: %s", SDL_GetError());
        return NULL;
    }
...
static char *BR_CMETHOD_DECL(br_device_sdl2, identifier)(struct br_object *self)
{
...
static br_int_32 BR_CMETHOD_DECL(br_device_sdl2, space)(struct br_object *self)
{
    (void)self;
    return (br_int_32)sizeof(br_device);
}
