...
br_error SDLToBRenderPixelFormat(Uint32 format, br_int_32 *bpp, br_uint_8 *type)
{
    ...
    BrWarning("SDL2", "Skipping unsupported pixel format %s", SDL_GetPixelFormatName(format));
    return BRE_FAIL;
}
...
br_output_facility *OutputFacilitySDL2CreateFromMode(br_device *dev, SDL_DisplayMode *mode, int monitor)
{
    ...
    BrSprintfN(tmp, sizeof(tmp) - 1, "%d:%dx%dx%d", monitor, mode->w, mode->h, bpp);
    ...
    for(int d = 0; d < ndisplays; ++d) {
        ...
        for(int m = 0; m < nmodes; ++m) {
            ...
            if(SDL_GetDisplayMode(d, m, &mode) < 0) {
                BrWarning("SDL2", "Unable to get display mode (display=%d,mode=%d)", d, m);
                continue;
            }
...
static char *BR_CMETHOD_DECL(br_output_facility_sdl2, identifier)(br_object *self)
{
...
static br_int_32 BR_CMETHOD_DECL(br_output_facility_sdl2, space)(br_object *self)
{
    return (br_int_32)sizeof(br_output_facility);
}
                                                                      ...
                                                                      br_device_pixelmap **ppmap, br_token_value *tv)
{
    ...
    if(is_window && is_surface) {
        BrWarning("SDL2", "Invalid parameter combination, refusing to create pixelmap");
        return BRE_FAIL;
    }
    ...
    if(is_window) {
        if(is_new) {
            ...
            if(window == NULL) {
                BrWarning("SDL2", "Error creating window: %s", SDL_GetError());
                return BRE_FAIL;
            }
        } else {
            window = pt.window;
