#ifndef sdl3rend_dyn_h
#define sdl3rend_dyn_h

/* SDL3 symbol table for the sdl3rend driver: one typedef and one extern
 ...
 *
 * SDL3REND_LoadSDLSymbols() (sdl3_dyn.c) must be called before any SDL3 use;
 * the driver does this in SDL3REND_VideoOpen(), passing the sdl3_handle field
 * of the br_device_sdl3_callback_procs structure.
 */

#include "sdl3_syms.h"

#define SDL3REND_X_TYPEDEF(name, ret, args) \
    typedef ret SDLCALL tSDL3REND_##name##_fn args;
#define SDL3REND_X_EXTERN(name, ret, args) \
    extern tSDL3REND_##name##_fn* SDL3_##name;

FOREACH_SDL3REND_SYM(SDL3REND_X_TYPEDEF)
FOREACH_SDL3REND_SYM(SDL3REND_X_EXTERN)
 ...
 * SDL3REND_SDL_DYNAMIC this binds the pointers directly to the SDL3 functions
 * and `handle` is ignored. */
int SDL3REND_LoadSDLSymbols(void* handle);

#endif /* sdl3rend_dyn_h */
