...
void BR_ASM_CALL RenderConvert2(struct brp_block *block, ...)
{
    ...
    if(m)
        for(c = 0; m; c++, m >>= 1)
            if(m & 1) {
                outv[0].comp_f[c] = FCONV(v0->comp[c]);
                printf("outv[0].comp_f[%d] = %f\n", c, outv[0].comp_f[c]);
                outv[1].comp_f[c] = FCONV(v1->comp[c]);
                printf("outv[1].comp_f[%d] = %f\n", c, outv[1].comp_f[c]);
            }
    ...
    if(m)
        for(c = 0; m; c++, m >>= 1)
            if(m & 1) {
                outv[0].comp_x[c] = XCONV(v0->comp[c]);
                printf("outv[0].comp_x[%d] = %d\n", c, outv[0].comp_x[c]);
                outv[1].comp_x[c] = XCONV(v1->comp[c]);
                printf("outv[1].comp_x[%d] = %d\n", c, outv[1].comp_x[c]);
            }
    ...
    if(m)
        for(c = 0; m; c++, m >>= 1)
            if(m & 1) {
                outv[0].comp_i[c] = ICONV(v0->comp[c]);
                printf("outv[0].comp_i[%d] = %d\n", c, outv[0].comp_i[c]);
                outv[1].comp_i[c] = ICONV(v1->comp[c]);
                printf("outv[1].comp_i[%d] = %d\n", c, outv[1].comp_i[c]);
            }
    ...
    if(m = 0x0000ffff ^ (rend.block->convert_mask_i | rend.block->convert_mask_x | rend.block->convert_mask_f))
        for(c = 0; m; c++, m >>= 1)
            if(m & 1) {
                ...
                printf("(2) outv[0].comp_i[%d] = %d\n", c, outv[0].comp_i[c]);
                printf("(2) outv[1].comp_i[%d] = %d\n", c, outv[1].comp_i[c]);
            }
